...
function useHistoryStateNavigation(base) {
	...
	function push(to, data) {
		...
		if (process.env.NODE_ENV !== "production" && !history.state) warn("history.state seems to have been manually replaced without preserving the necessary values. Make sure to preserve existing history state if you are manually calling history.replaceState:\n\nhistory.replaceState(history.state, '', url)\n\nYou can find more information at https://router.vuejs.org/guide/migration/#Usage-of-history-state");
...
function registerGuard(activeRecordRef, name, guard) {
	...
	if (!record) {
		if (process.env.NODE_ENV !== "production") warn(`No active route record was found when calling \`${name === "updateGuards" ? "onBeforeRouteUpdate" : "onBeforeRouteLeave"}()\`. Make sure you call this function inside a component child of <router-view>. Maybe you called it inside of App.vue?`);
		return;
	}
	...
	(0, vue.onActivated)(() => {
		...
		if (process.env.NODE_ENV !== "production" && !newRecord) warn("No active route record was found when reactivating component with navigation guard. This is likely a bug in vue-router. Please report it.");
...
function withDeprecationWarning(next) {
	...
	return function() {
		if (!warned) {
			warned = true;
			warn("The `next()` callback in navigation guards is deprecated. Return the value instead of calling `next(value)`.");
		}
